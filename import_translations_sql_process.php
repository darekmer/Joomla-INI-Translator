...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['sqlFile'])) {
    ...
    $sqlQueries = explode(";\n", $sqlContent); // Zakładamy, że każde zapytanie kończy się średnikiem i nową linią
    ...
    foreach ($sqlQueries as $query) {
        if (!empty(trim($query))) {
            try {
                ...
                $statement->execute();
                $importedRows += $statement->rowCount();
            } catch (PDOException $e) {
                $message = "Błąd podczas importowania: " . $e->getMessage();
    ...
    if (empty($message)) {
        $message = "Pomyślnie zaimportowano tłumaczenia. Liczba zaimportowanych wierszy: $importedRows.";
    }
} else {
    $message = "Błąd: Brak pliku do importu.";
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title><?php echo $uiTranslations['import_result_title']; ?></title>
...
<div class="main">
    ...
    <h2><?php echo $uiTranslations['import_result_title']; ?></h2>
    <p class="alert <?php echo ($importedRows > 0) ? 'alert-success' : 'alert-danger'; ?>">
        <?php echo $message; ?>
    </p>
